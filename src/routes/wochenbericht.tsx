...
export const Route = createFileRoute("/wochenbericht")({
  head: () => ({
    meta: [
      { title: "Wochenbericht-Vorschau – Project Partner Compass" },
      {
        name: "description",
        content:
          "Vorschau des wöchentlichen Berichts: alle Teilprojekte nach Status, Partnerorganisation und Frist, inklusive Meilensteine.",
      },
      { property: "og:title", content: "Wochenbericht-Vorschau – Project Partner Compass" },
      {
        property: "og:description",
        content: "So sieht der Montagsbericht aus, bevor er verschickt wird.",
      },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
    links: [{ rel: "canonical", href: "/wochenbericht" }],
  }),
...
function WochenberichtSeite() {
  ...
  const gruppen: BerichtGruppe[] =
    gruppierung === "status" ? bericht.nachStatus : gruppierung === "partner" ? bericht.nachPartner : bericht.nachFrist;
  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-8">
      <header className="flex flex-col gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Wochenbericht – Vorschau</h1>
          <p className="text-sm text-muted-foreground">
            So sieht der Bericht aus, der montags morgens verschickt wird. Beispielkarten sind fiktiv.
          </p>
        </div>
        <Reiter />
      </header>
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-lg border border-border bg-card p-4">
          <p className="text-xs uppercase tracking-wide text-muted-foreground">Projekte im Bericht</p>
          <p className="mt-1 text-2xl font-semibold">{bericht.anzahlProjekte}</p>
        </div>
        <div className="rounded-lg border border-border bg-card p-4">
          <p className="text-xs uppercase tracking-wide text-muted-foreground">Offene Meilensteine</p>
      ...
      <p className="flex items-start gap-2 rounded-lg border border-border bg-surface p-3 text-sm text-muted-foreground">
        ...
        <span>
          ...
          Der automatische Versand startet, sobald eine eigene Absenderadresse eingerichtet ist. Diese Vorschau zeigt
                ...
                {gruppe.eintraege.map(({ projekt, fristText, meilensteine: eigene }) => (
                  <li key={projekt.id} className="rounded-lg border border-border bg-card p-4">
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      ...
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${AMPEL_KLASSEN[fristAmpel(projekt.naechste_frist)]}`}>
                      ...
                      <ul className="mt-3 flex flex-col gap-1 border-t border-border pt-3">
                        {eigene.map((m) => (
                          <li key={m.id} className="flex flex-wrap items-center justify-between gap-2 text-sm">
                            <span className={m.erledigt ? "text-muted-foreground line-through" : ""}>{m.titel}</span>
                            <span className="flex items-center gap-2">
                              ...
                              <span
                                className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                                  m.erledigt ? "bg-secondary text-secondary-foreground" : AMPEL_KLASSEN[fristAmpel(m.frist)]
                                }`}
                              >
