...
/**
 * PDF-Export eines Projekts samt Meilensteinen, Dokumenten und Partnerkontakten –
 * zum direkten Herunterladen oder zum Ablegen bei den Projektdokumenten.
 */
export function ProjektExport() {
              ...
              <ul className="space-y-1 text-sm">
                {kontakte.map((k) => (
                  <li key={k.id}>
                    ...
                    <span className="text-muted-foreground">
                      {[k.rolle, k.email, k.telefon].filter(Boolean).join(" · ") ||
                        "keine Kontaktdaten"}
                    </span>
                  </li>
                ))}
